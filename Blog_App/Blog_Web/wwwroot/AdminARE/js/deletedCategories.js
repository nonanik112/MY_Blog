$(document).ready(function () {
    ...
    $(document).on('click',
        ...
        function (event) {
            ...
            Swal.fire({
                title: 'Arşivden geri gelmesini istermisiniz',
                text: `${commentText} adlı katergori arşivden geri getirilecek!`,
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function (data) {
                            const undoDeleteCategoryResult = jQuery.parseJSON(data);
                            console.log(commentResult);
                            if (undoDeleteCategoryResult.ResultStatus === 0) {
                                Swal.fire(
                                    'Arşivden Geri Geldi!',
                                    `${undoDeleteCategoryResult.Message}`,
                                    'success'
                                );
                                ...
                                Swal.fire({
                                    ...
                                    text: `${undoDeleteCategoryResult.Message}`,
                                });
                            }
                        },
                        error: function (err) {
    ...
    $(document).on('click',
        ...
        function (event) {
            ...
            let categoryName = tableRow.find('td:eq(1)').text();
            Swal.fire({
                title: 'Kalıcı olarak silme işlemi yapmak istermisiniz? ',
                text: `${commentText} adlı katergori kalıcı olarak silinecektir!`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (hardDeletedCategoryResult.ResultStatus === 0) {
                                Swal.fire(
